...
export default function MSOAwardsCustomise() {
  const [content, setContent] = useState({ subsections: defaultSubsections });
  const [settings, setSettings] = useState({
    title: "MSO Awards",
    subtitle: "Building prestige and community authority.",
  });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await axios.get("/api/settings/mso-awards");
        const data = res.data;
        ...
        if (data?.settings) {
          setSettings((prev) => ({
            ...prev,
            title: data.settings.title || "MSO Awards",
            subtitle: data.settings.subtitle || "Building prestige and community authority.",
          }));
        }
      } catch (error) {
        console.error("Error loading MSO Awards:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await axios.put("/api/settings/mso-awards", { content, settings }, { headers: { "Content-Type": "application/json" }, withCredentials: true });
      alert("MSO Awards section saved successfully!");
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-sm text-gray-500">Loading MSO Awards settings...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">MSO Awards</h1>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
          Assign posts to each award category. 1. MSO Maharashtra 50 = hero (first post featured). 2–3 = sidebar. 4–5 = below.
        </p>
      </div>
          ...
          <div className="p-6 space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="MSO Awards"
                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded focus:outline-none focus:ring-2 focus:ring-amber-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                placeholder="Building prestige and community authority."
                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded focus:outline-none focus:ring-2 focus:ring-amber-500"
              />
            </div>

            <div className="space-y-6">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white">Award Categories & Posts</h3>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="mt-2 w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded focus:outline-none focus:ring-2 focus:ring-amber-500"
                      />
                    </div>
                    {(sub.posts || []).length < getPostLimit(subIndex) && (
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium bg-amber-600 hover:bg-amber-700 text-white rounded shrink-0 ml-4"
                      >
            ...
            <div className="flex justify-end pt-6 border-t border-gray-200 dark:border-gray-700">
              <button
                type="submit"
                disabled={saving}
                className="inline-flex items-center gap-2 px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-60 text-white text-sm font-bold rounded"
              >
                <Save size={16} />
                {saving ? "Saving..." : "Save"}
              </button>
