...
import { Sparkles, Save, Search, X, Plus, Trash2 } from "lucide-react";
import axios from "axios";

export default function SpotlightCustomise() {
  ...
  useEffect(() => {
    const loadSpotlightData = async () => {
      try {
        ...
        console.error("Error loading spotlight section:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put("/api/settings/spotlight", {
  ...
  const addPostToSpotlight = (post) => {
    if (!isPostInSpotlight(post._id)) {
      setSpotlightContent((prev) => [...prev, post]);
      setSearchQuery("");
  ...
  const movePost = (index, direction) => {
    const newContent = [...spotlightContent];
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Loading spotlight section...</p>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Manage featured posts displayed in the spotlight section
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Add posts to display in the spotlight section
              </p>
            </div>
            <button
              type="button"
            ...
            <div className="mb-6 bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              <div className="max-h-64 overflow-y-auto border border-gray-200 dark:border-gray-800 rounded bg-white dark:bg-gray-900">
                {searchQuery ? (
                  <>
                    {filteredPosts
                      .filter((post) => !isPostInSpotlight(post._id))
                      .slice(0, 10)
                      .map((post) => {
                    ...
                    {filteredPosts.filter((post) => !isPostInSpotlight(post._id))
                      .length === 0 && (
                  ...
                  <div className="p-8 text-center">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      Search posts by title to add them to spotlight
                    </p>
                  </div>
                )}
              </div>
          ...
          {spotlightContent.length > 0 ? (
            <div className="space-y-2">
              {spotlightContent.map((post, index) => (
                <div
                  key={post._id}
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {index < spotlightContent.length - 1 && (
                        <button
                          type="button"
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 p-8 rounded border border-gray-200 dark:border-gray-700 text-center">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                No posts in spotlight
              </p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                Click "Add Post" to add posts to the spotlight section
              </p>
            </div>
          )}
        </div>
