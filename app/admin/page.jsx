...
export default function AdminPosts() {
  ...
  useEffect(() => {
    setPage(1);
  }, [statusFilter, categoryFilter, authorFilter, sortOrder, searchQuery]);
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (p) =>
          ...
          p.author?.companyName?.toLowerCase().includes(q),
      );
    }
    list.sort((a, b) => {
  ...
  const authorName = (a) => {
    if (!a) return "Unknown";
    return (a.accountType === "startup" || a.accountType === "company") ? (a.companyName || a.name) : a.name;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-extrabold text-gray-900">Posts</h1>
          <p className="text-xs text-gray-400 mt-0.5">All posts from every user, sorted by date</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={async (e) => {
              if (!confirm("Create 10 dummy posts?")) return;
              const btn = e.target.closest("button") || e.target;
              const orig = btn.innerHTML;
              try { btn.disabled = true; btn.innerHTML = "<span>Creating...</span>";
                const res = await fetch("/api/admin/seed-posts", { method: "POST" });
                const d = await res.json();
                if (res.ok) { alert(`Created ${d.created} posts`); await fetchPosts(); } else { alert(d.error || "Failed"); }
              } catch (err) { alert(err.message); } finally { btn.disabled = false; btn.innerHTML = orig; }
            }}
            className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-bold border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
            <PlusCircle size={14} />
            Seed
          </button>
          <Link href="/admin/add-post" className="inline-flex items-center gap-1.5 bg-red-600 text-white px-4 py-2 text-sm font-bold tracking-wide hover:bg-red-700 transition-colors">
            <PlusCircle size={14} />
            Add Post
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <div className="bg-white border border-gray-200 p-3">
          ...
          <div className="text-2xl font-extrabold text-gray-900 mt-1">{categories.length}</div>
        </div>
        <div className="bg-white border border-gray-200 p-3">
          <div className="text-[10px] font-bold tracking-wider uppercase text-gray-500">Authors</div>
          <div className="text-2xl font-extrabold text-gray-900 mt-1">{authors.length}</div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {paginated.length === 0 ? (
                <tr><td colSpan="6" className="px-3 py-8 text-center text-sm text-gray-400">{hasFilters ? "No posts match filters." : "No posts found."}</td></tr>
              ) : paginated.map((post) => (
                <tr key={post._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-1">
                      ...
                      <Link href={`/admin/edit-post/${post._id}`} className="p-1.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors" title="Edit"><Edit size={14} /></Link>
                      <button onClick={async () => {
                        if (!confirm("Delete this post?")) return;
                        try { const r = await fetch(`/api/superadmin/posts?postId=${post._id}`, { method: "DELETE" }); if (r.ok) await fetchPosts(); else { const d = await r.json(); alert(d.error || "Failed"); }
                        } catch { alert("Failed to delete"); }
                      }} className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
