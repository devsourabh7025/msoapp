          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="space-y-4">
                {[...Array(4)].map((_, j) => (
                  <div key={j} className="h-4 bg-gray-200 dark:bg-gray-800 animate-pulse" />
                ))}
              </div>
            ))}
          </div>
                ...
                {colPosts.map((post, index) => (
                  <li key={post._id || post.slug || index}>
                    <Link
                      ...
                      className="block text-lg font-medium leading-snug hover:underline text-gray-900 dark:text-white"
                    >
                      {post.title}
                    </Link>
                  </li>
