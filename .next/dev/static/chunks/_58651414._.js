...
"[project]/lib/fetchWithCache.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function fetchWithCache(url, options = {}) {
    ...
    const timeout = 15000; // 15 seconds timeout (increased to allow for slow DB connections)
    ...
    const requestPromise = (async ()=>{
        try {
            ...
            // Check if response was aborted before parsing
            if (controller.signal.aborted) {
                return {
                    ok: false,
                    data: null,
            ...
            if (error.name === 'AbortError' || error.name === 'TimeoutError' || controller.signal.aborted) {
                // Use console.warn instead of console.error for timeouts (expected behavior)
                return {
                    ok: false,
                    data: null,
                    error: 'Request timeout',
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$client$2f$app$2d$dir$2f$link$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/client/app-dir/link.js [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$fetchWithCache$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/fetchWithCache.js [app-client] (ecmascript)");
;
var _s = __turbopack_context__.k.signature();
"use client";
;
        ...
        "HeroFeatured.useEffect": ()=>{
            ...
            const loadHeroContent = {
                "HeroFeatured.useEffect.loadHeroContent": async ()=>{
                    try {
                        setLoading(true);
                        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$fetchWithCache$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["fetchWithCache"])("/api/settings/homepage");
                        if (!isMounted) return;
                        // Debug logging
                        console.log("🔍 HeroFeatured API Response:", {
                            ok: result.ok,
                            hasData: !!result.data,
                            content: result.data?.content,
                            hero: result.data?.content?.hero
                        });
                        if (result.ok && result.data) {
                            const heroData = result.data.content?.hero;
                            console.log("🔍 Hero Data:", heroData);
                            if (heroData) {
                                console.log("✅ Setting hero content:", {
                                    hasTopPicks: !!heroData.topPicks,
                                    topPicksLength: heroData.topPicks?.length || 0,
                                    hasMainArticle: !!heroData.mainArticle,
                                    hasDiscussionTable: !!heroData.discussionTable,
                                    discussionTableLength: heroData.discussionTable?.length || 0
                                });
                                setHeroContent(heroData);
                            } else {
                                console.warn("⚠️ No hero data found in response");
                                setHeroContent({});
                            }
                        } else {
                            console.warn("⚠️ API response not OK or no data:", result);
                            setHeroContent({});
                        }
                    } catch (error) {
            ...
            const loadSpotlightContent = {
                "SpotLight.useEffect.loadSpotlightContent": async ()=>{
                    try {
                        ...
                        if (!isMounted) return;
                        if (result.ok && result.data) {
                            if (result.data.content && result.data.content.spotlight) {
                                setSpotlightContent(result.data.content.spotlight);
                                return;
                            }
                        }
                    } catch (error) {
            ...
            const loadFeaturedContent = {
                "Featured.useEffect.loadFeaturedContent": async ()=>{
                    try {
                        ...
                        if (!isMounted) return;
                        if (result.ok && result.data) {
                            if (result.data.content && result.data.content.featured) {
                                setFeaturedContent(result.data.content.featured);
                                return;
                            }
                        }
                    } catch (error) {
        ...
        "TrendingNow.useEffect": ()=>{
            ...
            const loadTrendingContent = {
                "TrendingNow.useEffect.loadTrendingContent": async ()=>{
                    try {
                        const { fetchWithCache } = await __turbopack_context__.A("[project]/lib/fetchWithCache.js [app-client] (ecmascript, async loader)");
                        const result = await fetchWithCache("/api/settings/homepage");
                        if (!isMounted) return;
                        if (result.ok && result.data) {
                            if (result.data.content && result.data.content.trending) {
                                setTrendingContent(result.data.content.trending);
                                return;
                            }
                        }
                    } catch (error) {
            ...
            const loadDiscoverPosts = {
                "DiscoverMore.useEffect.loadDiscoverPosts": async ()=>{
                    try {
                        ...
                        const response = await fetch("/api/public/posts?limit=100");
                        if (!response.ok) {
...
//# sourceMappingURL=_58651414._.js.map